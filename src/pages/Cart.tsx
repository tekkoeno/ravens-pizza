...
const Cart = () => {
  ...
  const totalCount = items.reduce((obj, sum) => {
    return sum.count + obj;
  }, 0);
  const onClickClear = () => {
    if (window.confirm('Вы желаете очистить корзину?')) {
      dispatch(clearItem());
    }
  };
  const onClickPopup = () => {
  ...
  if (orderSent) {
    return <CartSuccess name={valueName} adress={valueAdress} />;
  }
  return (
    <>
      {open && (
        <div className="cart__popup" ref={windowRef}>
              ...
              <ul className="cart__popup-list">
                {items.map((item, index) => {
                  return (
                    <li key={index}>
                      <span>
                        <img src={item.imageUrl} alt="img-pizza" />
                      </span>
                      {item.name}
            ...
            <div className="cart">
              ...
              <div className="cart__bottom">
                ...
                <div className="cart__bottom-buttons">
                  ...
                  <button ref={popupRef} className="button pay-btn" onClick={onClickPopup}>
